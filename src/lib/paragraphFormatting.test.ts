...
describe('paragraph formatting',()=>{
 ...
 it('migrates schema 3 paragraph data to Legacy idempotently without content or ID changes',()=>{
   ...
   expect(migrated?.schemaVersion).toBe(7);expect(migrated?.project.typography?.paragraphs.presetId).toBe('legacy');expect(migrated?.project.chapters).toEqual(before);expect(migrateStoredProject(migrated)).toEqual(migrated);
 });
});
