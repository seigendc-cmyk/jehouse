...
export function computeBookProjectChecksum(project: unknown): string {
...
export function validateBookDataPackCompatibility(
  ...
  if (
    pack.checksum &&
    pack.projectData &&
    typeof pack.projectData === 'object' &&
    pack.checksum !== computeBookProjectChecksum(pack.projectData)
  ) {
    errors.push('Data-pack checksum mismatch.');
  }
...
export function createBookDataPack(project: BookProject) {
  ...
  return {
    ...
    checksum: computeBookProjectChecksum(project),
    metadata: {
      id: project.id,
      title: project.title,
