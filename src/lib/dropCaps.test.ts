...
describe('semantic drop caps',()=>{
 ...
 it('migrates legacy records idempotently without IDs, text, or appearance changes',()=>{const s=setup(),record:any={schemaVersion:4,projectId:s.project.id,localRevision:0,project:{...s.project,typography:{...s.project.typography,dropCaps:undefined}},createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),lastSavedAt:new Date().toISOString(),syncStatus:'local-only'};const before=structuredClone(record.project.chapters),m=migrateStoredProject(record)!;expect(m.schemaVersion).toBe(7);expect(m.project.typography!.dropCaps).toEqual(LEGACY_DROP_CAPS);expect(m.project.chapters).toEqual(before);expect(migrateStoredProject(m)).toEqual(m);});
